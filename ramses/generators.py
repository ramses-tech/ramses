...
from .utils import (
    ...
    dynamic_part_name, resource_schema,
...
def generate_server(raml_root, config):
    ...
    root_key = 'root'
    generated_resources = {root_key: config.get_root_resource()}
    ...
    for raml_resource in raml_root.resources:
        ...
        if parent_raml_res is not None:
            ...
            parent_resource = generated_resources[root_key]
        ...
        clean_uri = route_name = resource_uri.strip('/')
        log.info('Configuring resource: `{}`. Parent: `{}`'.format(
            route_name, parent_resource.uid or root_key))
